...
TEST_F(OpenMPIRBuilderTest, FuseLoopRangeNoDeadBlocks) {
  ...
  // Create three canonical loops sequentially.
  ...
  // Fuse the first two loops, leaving the third unfused.
  CanonicalLoopInfo *Fused = OMPBuilder.fuseLoops(DL, {Loop1, Loop2});

  OMPBuilder.finalize();
  EXPECT_FALSE(verifyModule(*M, &errs()));

  // Verify there are no unreachable blocks (blocks with no predecessors other
